...
- Internal tooling changes
  - [Update custom CSF tooling to the unified mutation API](https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#internal-csf-tools-use-the-unified-mutation-api)
- [Changes for addon authors](../addons/addon-migration-guide.mdx)

Storybook accepts prerelease Node.js builds when their version meets the minimum requirement. For example, Node.js 26.1.0-rc.0 is supported, but 22.12.0-rc.0 is older than 22.12.0 and is not supported.
...
Storybook 11 removes the deprecated `parameters.componentSubtitle` fallback. Move subtitles to `parameters.docs.subtitle` in your preview configuration and story files:
...
If `parameters.docs` already exists, add `subtitle` while keeping its other options. The upgrade command offers the `component-subtitle` automigration when it can preserve the existing behavior. If it reports an inherited subtitle conflict or an expression it cannot safely move, migrate the affected values manually.

Open the affected Docs pages and check that their subtitles remain correct. See the [full migration notes](https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#parameterscomponentsubtitle-removed) for the standalone automigration command and more examples.
